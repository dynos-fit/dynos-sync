...
class HeavyQueue implements QueueStore {
  ...
  Future<List<SyncEntry>> getPending({int limit = 50, DateTime? now}) async =>
      [];
  @override
  Future<Set<String>> getPendingIds(String t) async => {};
  @override
  Future<List<SyncEntry>> getPendingEntries(String t, String id) async => [];
  @override
  Future<bool> hasPending(String t, String i) async => false;
  @override
  ...
  group('DDOS & FLOOD RESILIENCE (100,000 Parallel Writes)', () {
    ...
    test('Engine stability under RLS-Bypass Flood Attempt', () async {
      ...
      for (var i = 0; i < 10000; i++) {
        attacks.add(engine
            .write('tasks', '$i', {'user_id': 'malicious'}).catchError((_) {}));
      }
      ...
      print(
          'RLS FLOOD: Ingested 10,000 unauthorized writes (all rejected). Event loop stable.');
    });
  });
}
