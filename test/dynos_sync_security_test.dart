...
class TrackingLocalStore implements LocalStore {
  ...
  @override 
  Future<void> upsert(String t, String id, Map<String, dynamic> d) async => data['$t:$id'] = d;
  @override 
...
class ZeroLatancyRemote extends Mock implements RemoteStore {
  @override Future<void> push(any, any, any, any) async {}
  @override Future<void> pushBatch(any) async {}
  @override Future<List<Map<String, dynamic>>> pullSince(any, any) async => [];
...
void main() {
  ...
  group('CATEGORY 1 — DATA LEAKS (Cross-User Isolation)', () {
    ...
    test('1-2. After User A Logout: User B context MUST be a total void', () async {
      // SEVERITY: CRITICAL
      ...
      // AUDIT
      expect(local.data, isEmpty, reason: 'Local data must be purged on logout');
      expect((await queue.getPending()), isEmpty, reason: 'Sync queue must be purged on logout');
      expect((await timestamps.get('workouts')).millisecondsSinceEpoch, 0);
    });

    test('3. Disk Zero-Leak: Read raw database simulation and check for PII', () async {
      // SEVERITY: HIGH
      ...
      expect(dump, isNot(contains('attack@leak.com')), reason: 'No user data should survive logout anywhere in storage');
    });
  ...
  group('CATEGORY 2 — DATA LEAKS (Payload & Log Exposure)', () {
    ...
    test('6. Payload Regex: Auth tokens must NEVER leak into JSON push body', () async {
      // SEVERITY: HIGH
      ...
      final attack = {'title': 'My Token Is xox-token-123', 'token': 'abc-def-123'};
      await engine.write('tasks', '1', attack);
      
      final captured = verify(() => remote.push(any(), any(), any(), captureAny())).captured.first as Map<String, dynamic>;
    ...
    test('7-8. Log PII Scrubbing: Capturing error logs during failure', () async {
      // SEVERITY: HIGH
      ...
      when(() => remote.pushBatch(any())).thenThrow(Exception('DUMPING DATA: user@evil.com secret123'));
      await engine.write('users', '1', {'email': 'user@evil.com', 'password': 'secret123'});
  ...
  group('CATEGORY 3 — SECURITY (Injection & Tampering)', () {
    ...
    test('11. SQL Injection: identifiers and values must be literalized', () async {
      // SEVERITY: HIGH
      final local = TrackingLocalStore();
      final engine = SyncEngine(
        local: local,
      ...
      expect(local.data['tasks:1']['title'], payload['title'], reason: 'Injection string was not treated as literal');
    });
    ...
    test('14. Tampered Server Response: Rejecting userId mismatch on Pull', () async {
      // SEVERITY: HIGH
      final engine = SyncEngine(
        local: MockLocalStore(),
        remote: MockRemoteStore(),
  ...
  group('CATEGORY 4 — AUTH & AUTHORIZATION', () {
    ...
    test('19. SUPABASE RLS Bypass: Blocking spoofed userId locally', () async {
      ...
      print('AUDIT: Supabase RLS bypass attempt blocked before hitting network.');
    });
  ...
  group('CATEGORY 5 — CONFLICT RESOLUTION', () {
    ...
    test('21. Last-Write-Wins: Remote (newer) MUST beat Local (older)', () async {
      // SEVERITY: HIGH
      final local = TrackingLocalStore();
      final remote = MockRemoteStore();
      final engine = SyncEngine(
      ...
      // Local state is old
      await engine.write('tasks', '1', {'title': 'Old', 'updated_at': tOld.toIso8601String()});
      
      // Remote state is new
      when(() => remote.getRemoteTimestamps()).thenAnswer((_) async => {'tasks': tNew});
      ...
      expect(local.data['tasks:1']['title'], 'New', reason: 'Remote newer version should have won');
    });
    ...
    test('24. Conflict: Delete vs Update: Engine Resolution Policy', () async {
      ...
      // Remote has UPDATE
      when(() => remote.getRemoteTimestamps()).thenAnswer((_) async => {'tasks': DateTime.now().toUtc()});
      when(() => remote.pullSince(any(), any())).thenAnswer((_) async => [
        {'id': '1', 'title': 'Remote Revived', 'updated_at': DateTime.now().toUtc().toIso8601String()}
      ]);

      await engine.pullAll();
      
      expect(local.data.containsKey('tasks:1'), isFalse, reason: 'Local Delete must beat Remote Update to prevent ghost records');
    });
  });
  ...
  group('CATEGORY 6 — PERFORMANCE LOADING', () {
    ...
    test('26. Bulk Ingest Speed: 10,000 writes in under 5 seconds', () async {
      ...
      expect(sw.elapsed.inSeconds, lessThan(5), reason: 'Engine ingestion rate too slow for high-scale apps');
    });
    ...
    test('34. Exponential Backoff: delays must double (2, 4, 8)', () async {
      ...
      when(() => remote.push(any(), any(), any(), any())).thenThrow(Exception('Net'));
      ...
      expect(d2, greaterThan(d1), reason: 'Retry delays must grow exponentially');
    });
  });
...
class InMemoryQueueStore implements QueueStore {
  ...
  @override Future<List<SyncEntry>> getPending({int limit = 50, DateTime? now}) async => 
    _queue.where((e) {
      ...
      if (now != null && _next.containsKey(e.id) && _next[e.id]!.isAfter(now)) return false;
      return true;
    }).take(limit).toList();
  ...
  @override Future<List<SyncEntry>> getPendingEntries(String t, String i) async {
    final list = _queue.where((e) => e.table == t && e.recordId == i && !_synced.contains(e.id)).toList();
    return list.map((e) => e.copyWith(
  ...
  @override Future<void> setNextRetryAt(String id, DateTime ts) async => _next[id] = ts;
  ...
  @override Future<void> clearAll() async { _queue.clear(); _synced.clear(); _retries.clear(); _next.clear(); }
}

class InMemoryTimestampStore implements TimestampStore {
  final Map<String, DateTime> _map = {};
  @override Future<DateTime> get(String t) async => _map[t] ?? DateTime.fromMillisecondsSinceEpoch(0, isUtc: true);
  @override Future<void> set(String t, DateTime ts) async => _map[t] = ts;
}
