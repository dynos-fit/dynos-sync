...
class MaliciousRemote extends Mock implements RemoteStore {
  ...
  @override
  Future<void> push(
      String t, String i, SyncOperation o, Map<String, dynamic> d) async {
...
void main() {
  ...
  group('PENETRATION TEST (High-Security Audit)', () {
    ...
    test('ATTACK 2: Data Exfiltration (PII Masking Audit)', () async {
      ...
      final attack = {
        'userId': 'hacker',
        'auth_token': 'EY-eyJhbGciOiJIUzI1...'
      };
      ...
      print(
          'PEN-TEST: Data exfiltration blocked (PII masking caught injected token).');
    });
    ...
    test('ATTACK 5: Poison Pill Resilience (Availability)', () async {
      ...
      print(
          'PEN-TEST: Poison pill isolated; healthy records continue to sync.');
    });
  });
}
...
class InMemoryQueueStore implements QueueStore {
  ...
  Future<List<SyncEntry>> getPendingEntries(String t, String id) async => _queue
      .where((e) => e.table == t && e.recordId == id && !_synced.contains(e.id))
      .toList()
  ...
  Future<void> purgeSynced(
    ...
    _queue.removeWhere((e) => _synced.contains(e.id));
  }
...
class InMemoryTimestampStore implements TimestampStore {
  ...
  Future<DateTime> get(String table) async =>
      _map[table] ?? DateTime.fromMillisecondsSinceEpoch(0, isUtc: true);
  @override
  Future<void> set(String table, DateTime ts) async => _map[table] = ts;
}
